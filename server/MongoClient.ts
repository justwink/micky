...
export class MongoClient {
  ...
  constructor (client: Promise<Mongo>, dbName: string) {
   ...
   */
  async insertOne (collectionNmae: string, doc:any, options = {}): Promise<IInsertOneResult> {
  ...
  async find (collectionName: string, query: object, fields: object, sortObj: any, pageObj: IPageObj, options = {}): Promise<Cursor<any>> {
    const collection = await this.getCollenction(collectionName)
    const { skip, limit } = this.getPagingInfo(pageObj)
   ...
   * @param {object} options 
   */
  async findOne (collectionName: string, query:any, fields = {}, options = {}): Promise<any> {
    const collection = await this.getCollenction(collectionName)
   ...
   * @param {*} options 
   */
  async findOneAndDelete (collectionName: string, filter:any, options = {}): Promise<FindAndModifyWriteOpResultObject> {
    const collection = await this.getCollenction(collectionName)
   ...
   * @param {object} options 
   */
  async findOneAndReplace (collectionName: string, filter:any, replacement:any, options = {}): Promise<FindAndModifyWriteOpResultObject> {
    const collection = await this.getCollenction(collectionName)
   ...
   * @param {object} options 
   */
  async findOneAndUpdate (collectionName: string, filter:any, update:any, options = {}): Promise<FindAndModifyWriteOpResultObject> {
    const collection = await this.getCollenction(collectionName)
   ...
   */
  async updateMany (collectionName: string, filter:any, update:any, options = {}): Promise<UpdateWriteOpResult> {
    const collection = await this.getCollenction(collectionName)
